...
mod tests {
    ...
    fn drain_emits_lifecycle_spans_while_recording() {
        ...
        assert!(
            label.contains("profiled.png") && label.contains("5×4"),
            "{label}"
        );
        let decode = data
            .spans
        ...
        assert!(!data.spans.iter().any(|s| {
            s.label
                .as_deref()
                .is_some_and(|l| l.contains("still decoding"))
        }));
    }
    ...
    fn scaled_variants_serve_at_the_requested_size() {
        ...
        let corner = with_scaled_image("mem://scaled", 8, 8, [4.0; 4], &[], |p| {
            p.pixels()[0].alpha()
        });
    ...
    fn confirmed_equal_srcs_share_pixels_without_a_decode() {
        ...
        assert_eq!(
            status("/nonexistent/other.png", &missing),
            ImageStatus::Pending
        );
    }
    ...
    fn insert_decoded_serves_without_a_file() {
        ...
        let alpha_seen = with_image("mem://test", |p| {
            p.pixels().iter().any(|px| px.alpha() < 255)
        });
        assert_eq!(alpha_seen, Some(true));
    }
}
