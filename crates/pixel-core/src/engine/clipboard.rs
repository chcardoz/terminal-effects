...
mod tests {
    ...
    fn parse_round_trips_and_tolerates_unmatched_delimiters() {
        let m = MARK_CHAR;
        let pasted = format!(
            "x{MARK_TOKEN_OPEN}one{MARK_TOKEN_CLOSE}y{MARK_TOKEN_OPEN}two{MARK_TOKEN_CLOSE}"
        );
        let (text, marks) = parse_rich_paste(&pasted).unwrap();
        assert_eq!(text, format!("x{m}y{m}"));
        assert_eq!(
            marks,
            vec![(1, "one".into()), (2 + m.len_utf8(), "two".into())]
        );

        assert!(parse_rich_paste("plain text").is_none());
        let unmatched = format!("a{MARK_TOKEN_OPEN}never closed");
        assert!(
            parse_rich_paste(&unmatched).is_none(),
            "unmatched keeps text plain"
        );
    }
}
