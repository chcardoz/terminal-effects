...
impl Engine {
    ...
    pub(super) fn handle_doc_key(&mut self, key: &KeyEvent) -> io::Result<bool> {
        ...
        let handled = match key.key {
            ...
            crate::terminal::Key::Home if m.shift => {
                self.comp.views[view].tree.doc_extend(true, Line)
            }
            crate::terminal::Key::End if m.shift => {
                self.comp.views[view].tree.doc_extend(false, Line)
            }
    ...
    pub(super) fn begin_text_selection(&mut self, view: usize, local: (f32, f32)) {
        ...
        } else if self.comp.views[view]
            .tree
            .doc_select_down_near(local, fonts)
        {
            self.drag = Some((view, DragTarget::Text));
        } else {
