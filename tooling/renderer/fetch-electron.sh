...
BROWSER="$REPO_ROOT/apps/renderer/browser"
VERSION="$(node -e 'console.log(require(process.argv[1]+"/package.json").devDependencies.electron)' "$BROWSER")"
DEST="$(node -e 'const p=require("path");console.log(p.join(p.dirname(require.resolve("electron/package.json",{paths:[process.argv[1]]})),"dist"))' "$BROWSER")"
