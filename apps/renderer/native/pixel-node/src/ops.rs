...
use pixel_core::{
    ...
    Overflow, Position, Props, ScrollbarStyle, SelectionMode, ShapeProps, ShapeStroke, SlotKind,
    Style, TextSpan, parse_path_data,
};
...
fn register_font(engine: &mut Engine, path: &str) -> String {
    let loaded = std::fs::read(path)
        ...
        .and_then(|bytes| {
            pixel_core::fontdue::Font::from_bytes(
                bytes,
                pixel_core::fontdue::FontSettings::default(),
            )
...
impl StyleDto {
    fn into_style(self, rem: f32) -> Style {
        Style {
            ...
            corner_radius: self
                .corner_radius
                .map_or([0.0; 4], CornerRadiusDto::into_radii),
    ...
    if let Some(tree) = engine.comp.views.get(view).map(|v| &v.tree) {
        ...
        while let Some(id) = stack.pop() {
            ...
            if let Some(metrics) = tree.box_metrics(id) {
                let edges =
                    |e: Edges| json!({ "l": e.left, "t": e.top, "r": e.right, "b": e.bottom });
