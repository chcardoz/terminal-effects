...
extern "C" fn sigwinch_handler(_: libc::c_int) {
    for slot in &RESIZE_WAKE_FDS {
        ...
        if fd >= 0 {
            // SAFETY: `write` is async-signal-safe and the registered descriptor
            // is the write end of a live wake pipe while it occupies this slot.
            unsafe {
...
impl Terminal {
    ...
    fn write_frame_file(&mut self, data: &[u8]) -> io::Result<String> {
            ...
            .is_none_or(|file| file.len != data.len())
        {
            ...
            for slot in 0..FRAME_SLOTS {
                self.frame_files.push(FrameFile::create(
                    self.frame_path(slot, generation),
                    data.len(),
                )?);
            }
        }
    ...
    pub fn watch_resize(&mut self) -> io::Result<()> {
        ...
        // SAFETY: the action is initialized before registration and points to an
        // `extern "C"` handler with the signal-handler ABI required by sigaction.
        unsafe {
    ...
    fn probe_kitty_keyboard(&mut self) -> io::Result<bool> {
        ...
        self.io.out().flush()?;
        Ok(self
            .read_report(150, parse_kitty_keyboard)?
            .unwrap_or(false))
    }
...
pub(crate) fn write_shm(name: &str, data: &[u8]) -> io::Result<()> {
    ...
    // SAFETY: mmap returns a writable region of `data.len()` bytes; the source
    // slice is valid and non-overlapping, and the mapping is unmapped once here.
    unsafe {
...
mod tests {
    ...
    fn shm_roundtrip() {
        ...
        .unwrap();
        // SAFETY: the test maps exactly the initialized shared-memory length,
        // copies it before unmapping, and does not retain a view afterward.
    ...
    fn natural_text_editing_bytes_unrewrite_only_under_kitty() {
        ...
        assert_eq!(
            parse_event_kitty(b"\x01", true),
            Some((key_mods(Key::Left, SUPER), 1))
        );
        assert_eq!(
            parse_event_kitty(b"\x05", true),
            Some((key_mods(Key::Right, SUPER), 1))
        );
        assert_eq!(
    ...
    fn kitty_keyboard_reply_is_the_whole_answer() {
        ...
        assert_eq!(
            parse_kitty_keyboard(b"\x1b[?62;22;52c\x1b[?27u"),
            Some(true)
        );
    }
    ...
    fn terminal_reply_strings_never_leak_as_keystrokes() {
        ...
        assert_eq!(
            ...
            Some((
                RawEvent::Color(ColorSlot::Background, [30, 42, 52, 255]),
                18
            )),
    ...
    fn reads_color_replies_for_every_slot() {
        assert_eq!(
            ...
            Some((
                RawEvent::Color(ColorSlot::Foreground, [255, 238, 221, 255]),
                19
            ))
        );
        assert_eq!(
            ...
            Some((
                RawEvent::Color(ColorSlot::Palette(13), [159, 134, 235, 255]),
                27
            ))
        );
        assert_eq!(
    ...
    fn reads_the_color_scheme_notification() {
        assert_eq!(
            parse_event(b"\x1b[?997;1n"),
            Some((RawEvent::ColorSchemeChanged, 9))
        );
        assert_eq!(
            parse_event(b"\x1b[?997;2n"),
            Some((RawEvent::ColorSchemeChanged, 9))
        );
    }
    ...
    fn parses_kitty_associated_text_and_function_keys() {
        ...
        assert_eq!(parse_event(b"\x1bOP"), Some((key(Key::Function(1)), 3)));
        assert_eq!(
            parse_event(b"\x1b[57444;9u"),
            Some((key_mods(Key::LeftSuper, SUPER), 10))
        );
    }
    ...
    fn clip_packets_parse_status_mime_and_chunk() {
        ...
        assert!(
            parse_clip_packet(b"\x1b]52;c;?\x1b\\").is_none(),
            "osc52 untouched"
        );
    }
...
mod tty_tests {
    ...
    fn open_pty() -> (std::fs::File, std::fs::File, String) {
        ...
        #[allow(unsafe_code)]
        // SAFETY: all output pointers reference valid writable storage, the
        // name buffer is large enough for a platform PTY path, and optional
        // termios/winsize inputs are intentionally null.
        let ok = unsafe {
            libc::openpty(
                &mut master,
                &mut slave,
        ...
        #[allow(unsafe_code)]
        // SAFETY: successful `openpty` returned two new owned descriptors, each
        // transferred into exactly one File.
    ...
    fn two_terminals_share_a_process() {
        ...
        assert!(
            other.is_none(),
            "terminal B must not see A's input: {other:?}"
        );
    ...
    fn a_bare_escape_still_waits_for_more_outside_tmux() {
        ...
        assert!(
            got.is_none(),
            "outside tmux escape arrives as CSI-u: {got:?}"
        );
    }
