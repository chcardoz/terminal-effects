...
fn blend_row(dst: &mut [u8], src: &[u8]) {
    ...
    if src_pixels.iter().all(|s| s[3] == 255) {
        dst.copy_from_slice(src);
        return;
    }
    let (dst_pixels, _) = dst.as_chunks_mut::<4>();
    for (dst, s) in dst_pixels.iter_mut().zip(src_pixels) {
        match s[3] {
...
impl Canvas {
    ...
    fn occluded(&self, x: f32, y: f32, w: f32, h: f32) -> bool {
        let inside = self.occluders.iter().any(|&(_, (ox, oy, ow, oh))| {
    ...
    fn fill_rows(&mut self, x1: u32, y1: u32, x2: u32, y2: u32, color: [u8; 4]) {
        ...
        for px in self.pixels[first..first + row_len].as_chunks_mut::<4>().0 {
...
impl Canvas {
    ...
    fn fill_large_rounded_rect(
        ...
        if h < strip * 2.0 + 2.0 {
            ...
            if let Some(path) = rounded_rect_path(x, y, strip, h, [radius[0], 0.0, 0.0, radius[3]])
            {
    ...
    pub fn stroke_rounded_rect(
        ...
        if w * h >= 65536.0 && w > corner * 2.0 + 2.0 && h > corner * 2.0 + 2.0 {
            ...
            if let Some(path) =
                rounded_rect_path(x + inset, y + inset, w - width, h - width, stroke_radius)
            {
    ...
    fn blend_fill(&mut self, x: f32, y: f32, w: f32, h: f32, color: [u8; 4]) {
        ...
        for row in y1..y2 {
            ...
            for px in self.pixels[start..start + len].as_chunks_mut::<4>().0 {
    ...
    pub fn blit_scaled_rgba_rounded(
        ...
        let hsample = |src_row: &[u8], out: &mut [u8]| {
            for (&(at, f), dst) in columns.iter().zip(out.as_chunks_mut::<4>().0) {
        ...
        for row in y1..y2 {
            ...
            for ((dst, a), b) in dst_row
                ...
                .0
                .iter_mut()
                .zip(top.1[from..].as_chunks::<4>().0)
                .zip(bottom.1[from..].as_chunks::<4>().0)
            {
                let mut s = [0u8; 4];
                for c in 0..4 {
...
mod tests {
    ...
    fn fills_inside_an_occluder_are_skipped_until_it_is_withdrawn() {
        ...
        assert_eq!(
            canvas.pixels[(3 * 10 + 3) * 4],
            0,
            "fill inside the occluder must be skipped"
        );

        canvas.fill_rounded_rect(0.0, 0.0, 2.0, 2.0, [0.0; 4], [255, 0, 0, 255]);
        assert_eq!(
            canvas.pixels[0], 255,
            "fill outside the occluder must paint"
        );

        canvas.fill_rounded_rect(1.0, 1.0, 4.0, 4.0, [0.0; 4], [0, 255, 0, 255]);
        assert_eq!(
            canvas.pixels[11 * 4 + 1],
            255,
    ...
    fn a_fill_straddling_an_occluder_paints_only_the_uncovered_bands() {
        ...
        assert_eq!(
            canvas.pixels[0], 255,
            "outside the occluder the fill paints"
        );
        ...
        assert_eq!(
            canvas.pixels[(2 * 10 + 1) * 4],
            255,
            "the band edge is exact"
        );
