...
fn current_thread_port() -> u32 {
    ...
    // SAFETY: both functions are pthread/Mach queries for the currently
    // executing thread and require no caller-owned pointers.
...
fn run_controller(inner: &Inner) {
    ...
    loop {
        ...
        for &port in snapshot.iter().filter(|&&p| p != 0) {
            // SAFETY: nonzero ports were published by live worker threads and
            // Mach handles stale ports by returning an error, which is ignored.
            unsafe {
                thread_suspend(port);
            }
        }
        ...
        for &port in snapshot.iter().filter(|&&p| p != 0) {
            // SAFETY: this resumes the same snapshot suspended above; Mach
            // reports invalid or stale ports without dereferencing memory.
            unsafe {
                thread_resume(port);
            }
        }
