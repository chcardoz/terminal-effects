...
pub fn resolve() -> Result<PathBuf> {
    let renderer = locate().with_context(|| {
            "the packaged Terminal Effects renderer is missing. Install a complete Terminal Effects release, run `pnpm install && pnpm build:runtime` from the repository root for development, or use `te serve .`"
        })?;
