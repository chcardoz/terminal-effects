...
static FONT_BYTES: &[u8] =
    include_bytes!("../../../../apps/renderer/assets/fonts/JetBrainsMono-Regular.ttf");
...
fn spans_color_glyph_runs_within_one_text_node() {
    ...
    assert!(
        any_pixel(cell, cell * 2, &|p| p[0] > 100
            && p[1] == p[0]
            && p[2] == p[0]),
...
fn selection_stays_inside_the_scroll_container_it_started_in() {
    ...
    tree.doc_select_drag(
        (left_rect.x + 2.0, left_rect.y + left_rect.h + 30.0),
        &fonts,
    );
    ...
    assert!(
        selected.contains("nel"),
        "selection extends within its own container: {selected:?}"
    );
...
fn mark_widgets_reserve_width_and_place_inline() {
    ...
    assert_eq!(
        marks[0].advance, 40.0,
        "widget width becomes the mark advance"
    );
