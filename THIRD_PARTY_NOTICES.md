# Third-party assets

The interface uses [lucide-react](https://www.npmjs.com/package/lucide-react),
copyright Lucide Contributors and distributed under the ISC license. The
dependency and exact version are recorded in `pnpm-lock.yaml`; no icon
files are copied into this repository.

The terminal renderer incorporates portions of
[terminal-browser](https://github.com/zenbu-labs/terminal-browser), copyright
Zenbu Labs, Inc., distributed under the MIT license. Its source is maintained in
`apps/renderer/`, and the license text is preserved in
`apps/renderer/LICENSE.terminal-browser`. Terminal Effects release archives bundle a
pinned Electron/Chromium runtime built by maintainers; users do not download or
install terminal-browser separately.
