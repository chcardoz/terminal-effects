...
impl Herdr {
    ...
    fn write_frame(&mut self, pixels: &[u8]) -> io::Result<String> {
        if self
            .files
            .first()
            .is_none_or(|file| file.len() != pixels.len())
        {
            self.generation += 1;
...
#[cfg_attr(
    ...
    expect(
        dead_code,
        reason = "herdr reports pane_visible; nothing skips rendering on it yet"
    )
)]
...
mod tests {
    ...
    fn an_info_reply_without_file_transport_is_not_ours_to_use() {
        let plain =
            r#"{"id":"info","result":{"cell_width_px":9,"cell_height_px":19,"pane_visible":true}}"#;
    ...
    fn a_resize_keeps_the_previous_pixels_until_the_next_frame_is_taken() {
        ...
        assert!(
            before.exists(),
            "herdr may still replay the placement it has"
        );
    ...
    fn frames_cycle_through_slots_so_herdr_reads_one_we_are_not_writing() {
        ...
        assert_eq!(
            paths.len(),
            SLOTS as usize,
            "consecutive frames shared a file"
        );
    }
    ...
    fn quoting_survives_a_path_with_characters_json_cares_about() {
        ...
        assert_eq!(
            field_str(&format!(r#"{{"p":{}}}"#, quote(r#"/a"b\c"#)), "p").as_deref(),
            Some(r#"/a"b\c"#)
        );
    }
}
...
mod shared_process_tests {
    ...
    fn engines_sharing_a_process_use_distinct_frame_files() {
        ...
        assert_ne!(
            path_a, path_b,
            "same-process engines must not share frame files"
        );
